...
class TestEnhancedCommercialData:
    ...
    def test_path_params_retrieve(self, client: Lithic) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `event_token` but received ''"):
            client.transactions.events.enhanced_commercial_data.with_raw_response.retrieve(
                "",
            )
...
class TestAsyncEnhancedCommercialData:
    ...
    async def test_path_params_retrieve(self, async_client: AsyncLithic) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `event_token` but received ''"):
            await async_client.transactions.events.enhanced_commercial_data.with_raw_response.retrieve(
                "",
            )
